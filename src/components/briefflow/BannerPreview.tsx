// components/briefflow/BannerPreview.tsx
...
export function BannerPreview({ state, onChange }: Props) {
  ...
  const url = useMemo(
    () => state.imagePrompt ? buildPollinationsUrl(state.imagePrompt, { width: 1200, height: 400, seed: state.imageSeed }) : null,
  ...
  return (
    <div className="mx-auto flex w-full flex-col space-y-4">
      <div className="relative flex aspect-[21/9] md:aspect-[3/1] min-h-[250px] w-full shrink-0 overflow-hidden rounded-2xl bg-[#0a0a0c] shadow-2xl ring-1 ring-border/50">
        {url ? (
          <>
            ...
            {/* Box de Texto com trava de largura (max-w-[50%]) e overflow hidden */}
      ...
      <div className="flex items-center justify-between rounded-xl border border-border/50 bg-background p-3 opacity-60 hover:opacity-100">
        ...
        <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => { setLoading(true); onChange({ imageSeed: Math.floor(Math.random() * 1_000_000) }); }}>
          <RefreshCw className="mr-2 size-3" /> Retry Image
        </Button>
      </div>
    </div>
  );
