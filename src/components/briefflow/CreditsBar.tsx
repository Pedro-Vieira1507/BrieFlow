...
export function CreditsBar({ className }: Props) {
  ...
  return (
    <div className={cn("px-5 py-3 border-b border-border-subtle bg-surface-2 flex flex-col gap-2 transition-colors", className)}>
      <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest">
        <span className="text-fg-secondary">
          Créditos ({planLabel(plan.plan)})
        </span>
        <span className={cn("transition-colors duration-300", isLow ? "text-rose-400 animate-pulse" : "text-fg-primary")}>
      ...
      <div className="relative h-1.5 w-full overflow-hidden rounded-full bg-surface-3 shadow-inner">
        <div
          className={cn(
            ...
            isLow ? "bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.6)]" : "bg-brand"
          )}
