...
export function ProfileSettingsModal({ open, onOpenChange }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn(
          "max-h-[94dvh] w-[calc(100vw-24px)] overflow-y-auto rounded-[24px] border-border-strong bg-surface-1 text-fg-primary shadow-[var(--shadow-elevated)] transition-all",
          isUpgrading ? "sm:max-w-[660px]" : "sm:max-w-[540px]",
        )}
      >
        {!isUpgrading && (
        ...
        {!isUpgrading ? (
          <Tabs defaultValue="ai" className="mt-1 w-full">
            <TabsList className="grid h-12 w-full grid-cols-2 rounded-xl border border-border-subtle bg-surface-2 p-1">
              <TabsTrigger
                value="ai"
                className="rounded-lg text-xs font-semibold data-[state=active]:bg-surface-3 data-[state=active]:text-brand data-[state=active]:shadow-sm sm:text-sm"
              >
            ...
            <TabsContent value="ai" className="space-y-5 mt-6 px-1">
              <div className="space-y-2">
                <Label className="text-fg-secondary font-semibold">
                  Persona Padrão
                </Label>
                <Input
                  value={persona}
                ...
                <p className="text-[12px] text-fg-muted mt-1">
                  Quem a IA deve assumir que é seu público principal.
                </p>
              </div>
              <div className="space-y-2">
                <Label className="text-fg-secondary font-semibold">
              ...
              <div className="flex justify-end pt-4">
                <Button
                  onClick={handleSave}
                  className="h-11 w-full rounded-xl bg-brand px-6 font-semibold text-brand-fg shadow-[var(--shadow-brand)] hover:brightness-110 sm:w-auto"
                >
            ...
            <TabsContent value="billing" className="space-y-5 mt-6 px-1">
              <div className="rounded-2xl border border-border-strong bg-gradient-to-b from-surface-2 to-surface-1 p-6 shadow-sm">
                <div className="mb-6 flex flex-col gap-4 border-b border-border-subtle pb-6 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h4 className="font-bold text-fg-primary text-sm uppercase tracking-wider mb-1">
                      Plano Atual
                    </h4>
                    <p className="text-xl font-display font-semibold text-brand">
                  ...
                  <div className="text-left sm:text-right">
                    ...
                    <p className="max-w-full truncate rounded-full bg-surface-3 px-3 py-1 text-sm font-medium text-fg-secondary sm:max-w-[240px]">
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative h-2.5 w-full overflow-hidden rounded-full bg-surface-3 shadow-inner">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, creditsPercent || 0)}%`,
                      }}
                    />
                  </div>
                </div>
              ...
              <div className="pt-2">
                <Button
                  ...
                  className="h-12 w-full rounded-xl bg-fg-primary font-bold text-surface-0 shadow-lg hover:bg-white"
                >
                  <Zap className="size-4 mr-2 fill-black" /> Fazer Upgrade do
                  Plano
                </Button>
              </div>
            </TabsContent>
          ...
          <div className="space-y-6 animate-in slide-in-from-right-8 duration-300 py-2">
            <div className="flex items-center gap-3 mb-2">
              <button
                ...
                className="grid size-9 place-items-center rounded-xl border border-border-subtle bg-surface-2 text-fg-muted shadow-sm transition-colors hover:bg-surface-3 hover:text-fg-primary"
              >
                <ArrowLeft className="size-4" />
              </button>
              <div>
                <h3 className="font-display font-bold text-fg-primary text-2xl">
                  Escolha seu plano
                </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto pr-1 max-h-[65vh] pb-4">
              ...
              <div className="border-2 border-brand/80 rounded-2xl p-6 bg-gradient-to-b from-brand/10 to-surface-2 relative overflow-hidden shadow-[0_10px_40px_-10px_rgba(99,102,241,0.2)] md:col-span-2">
                <div className="absolute top-0 right-0 bg-brand text-brand-fg text-[10px] font-black tracking-widest px-4 py-1.5 rounded-bl-xl uppercase shadow-md">
                  Mais Popular
                </div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h4 className="font-bold text-xl text-fg-primary leading-none">
                      Plano Pro
                    </h4>
                    <p className="text-sm text-brand font-medium mt-1">
                      O ideal para criadores
                    </p>
                  </div>
                </div>
                <p className="text-4xl font-black text-fg-primary mb-6 tracking-tight">
                  R$ 49,90
                  <span className="text-base font-semibold text-fg-muted tracking-normal">
                    /mês
                  </span>
                </p>
                <ul className="space-y-3.5 text-sm font-medium text-fg-secondary md:grid md:grid-cols-2 md:gap-x-4 md:space-y-0 md:gap-y-3">
                  <li className="flex items-start gap-3">
                    <Check className="size-5 text-brand shrink-0 mt-0.5" /> 1000
                    créditos renovados por dia
                  </li>
                  <li className="flex items-start gap-3">
                    <Check className="size-5 text-brand shrink-0 mt-0.5" />{" "}
                    Exportação de imagens em alta resolução
                  </li>
                  <li className="flex items-start gap-3">
                    <Check className="size-5 text-brand shrink-0 mt-0.5" />{" "}
                    Respostas prioritárias e sem filas
                  </li>
                  <li className="flex items-start gap-3">
                    <Check className="size-5 text-brand shrink-0 mt-0.5" />{" "}
                ...
                <Button
                  ...
                  className="w-full mt-8 bg-brand text-brand-fg hover:brightness-110 shadow-lg shadow-brand/25 rounded-xl h-12 font-bold text-base"
                >
                  Assinar Pro Agora
                </Button>
              </div>
              ...
              <div className="border border-border-strong rounded-2xl p-6 bg-surface-2/30 flex flex-col">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="font-bold text-lg text-fg-primary">
                    Gratuito
                  </h4>
                  {(!plan || plan?.plan === "free") && (
                    <span className="text-[10px] font-bold uppercase tracking-wider bg-surface-3 px-2 py-1 rounded-md text-fg-muted border border-border-subtle">
                      Atual
                    </span>
                  )}
                </div>
                <p className="text-2xl font-black text-fg-primary mb-6">
                  R$ 0
                ...
                <ul className="space-y-3 text-sm text-fg-secondary flex-1">
                  <li className="flex items-start gap-2">
                    <Check className="size-4 text-fg-muted shrink-0 mt-0.5" />{" "}
                    50 créditos diários
                  </li>
                  <li className="flex items-start gap-2">
                    <Check className="size-4 text-fg-muted shrink-0 mt-0.5" />{" "}
                    Geração de banners, e-mails e posts
                  </li>
                </ul>
              </div>
              ...
              <div className="border border-border-strong rounded-2xl p-6 bg-surface-2 flex flex-col">
                ...
                <ul className="space-y-3 text-sm text-fg-secondary flex-1">
                  ...
                  <li className="flex items-start gap-2">
                    <Check className="size-4 text-fg-muted shrink-0 mt-0.5" />{" "}
                    Múltiplas marcas configuráveis
                  </li>
                </ul>
                <Button
                  onClick={handleComingSoon}
                  variant="outline"
                  className="w-full mt-6 border-border-strong text-fg-primary hover:bg-surface-3 rounded-xl h-11 font-semibold"
                >
                  Assinar Agência
                </Button>
              </div>
