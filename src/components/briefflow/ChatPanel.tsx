...
import { Globe, Send, Sparkles, Loader2 } from "lucide-react";
    ...
    if (nextMatch.type === "image") {
      parts.push(
        <img
          ...
          src={nextMatch.url}
          alt={nextMatch.text}
          className="my-3 max-h-48 rounded-xl object-contain border border-white/10 bg-white/5 shadow-xl"
        />,
      );
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((step) => (
                <div
                  key={step}
                  className={`h-1.5 w-4 rounded-full transition-colors ${step <= currentStep ? "bg-blue-500" : "bg-white/10"}`}
                />
              ))}
            </div>
          </div>
        ...
        {messages.length === 0 && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col items-center text-center mt-12">
            ...
            <p className="text-sm text-white/50 mb-8 max-w-[280px] leading-relaxed">
              Sou sua IA de Direção de Arte. Diga o objetivo, cole o site da sua
              marca e eu crio as artes.
            </p>
            <div className="flex flex-col gap-3 w-full">
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => onSend(s)}
                  className="text-[13px] text-left px-5 py-3.5 rounded-xl border border-white/5 bg-white/5 hover:bg-white/10 transition-colors text-white/70 shadow-sm"
                >
                  "{s}"
                </button>
              ))}
      ...
      <div className="p-5 border-t border-white/5 bg-[#0E0E12]">
        <div className="relative rounded-2xl border border-white/10 bg-[#18181B] shadow-inner focus-within:border-blue-500/50 focus-within:ring-1 focus-within:ring-blue-500/50 transition-all">
          ...
          <Button
            ...
            className="absolute right-2 bottom-2 size-9 rounded-xl bg-blue-600 text-white hover:bg-blue-500 transition-transform"
          >
