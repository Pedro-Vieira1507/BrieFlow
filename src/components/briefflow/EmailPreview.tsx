...
export function EmailPreview({ state, onChange }: Props) {
  ...
  useEffect(() => { if (heroUrl) { setLoading(true); setError(false); } }, [heroUrl]);
        ...
        <div className="bg-white dark:bg-[#0c0c0e] rounded-2xl shadow-xl border border-slate-100 dark:border-white/5 overflow-hidden">
          ...
          {heroUrl && !error ? (
            <div className="relative aspect-[2.2/1] w-full bg-slate-100 dark:bg-slate-900 overflow-hidden">
              {loading && <div className="absolute inset-0 flex items-center justify-center z-10"><Loader2 className="size-8 animate-spin text-slate-400" /></div>}
              <img src={heroUrl} alt="Hero" onLoad={() => setLoading(false)} onError={() => { setLoading(false); setError(true); }}
                className={`h-full w-full ${isProductImage ? 'object-contain mix-blend-multiply bg-white p-8' : 'object-cover'}`} />
            </div>
          ) : null}
