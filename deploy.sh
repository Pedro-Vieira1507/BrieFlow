...
# ─── Configurações ────────────────────────────────────────────────────────────────
REPO_URL="https://github.com/Pedro-Vieira1507/BrieFlow.git"
...
# Configurar Ollama como serviço systemd (escuta apenas em localhost)
...
EOF_SERVICE
  ...
  sleep 3
  log "Ollama rodando como serviço systemd (só acessível internamente)"
else
  log "Ollama já está rodando"
fi

# Baixar modelo
info "Baixando modelo $OLLAMA_MODEL (pode demorar 5–20 min)..."
...
pm2 startup systemd -u "$USER" --hp "$HOME" | tail -1 | bash 2>/dev/null || \
  warn "Execute: pm2 startup systemd"
...
log "Caddy configurado como proxy reverso"

# ─── 11. Firewall (Oracle Cloud usa iptables além de Security Lists) ────────────────────
info "Abrindo portas no firewall..."
sudo iptables -I INPUT 6 -m state --state NEW -p tcp --dport 80 -j ACCEPT 2>/dev/null || true
sudo iptables -I INPUT 6 -m state --state NEW -p tcp --dport 443 -j ACCEPT 2>/dev/null || true
sudo netfilter-persistent save 2>/dev/null || true
log "Portas 80 e 443 abertas"
...
log "PM2 (briflow):   $(pm2 list 2>/dev/null | grep -E 'briflow.*online' | wc -l) instância(s) online"
...
warn "Caminho: Oracle Console → Networking → Virtual Cloud Networks → Security Lists"
