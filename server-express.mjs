...
const BRAND_IDENTITIES = {
  forlab:{
    displayName:"FORLAB",bg1:"#001f5b",bg2:"#003399",accent:"#0055cc",accentLight:"#4d90fe",
    badgeColor:"#0055cc",dotColor:"#4d90fe",font:"Montserrat",palette:"azul corporativo",
    productQueries:["FORLAB pipette laboratory equipment product white background","FORLAB micropipette single channel professional","FORLAB laboratory analytical equipment"],
  },
  shimadzu:{
    displayName:"SHIMADZU",bg1:"#001433",bg2:"#002d6b",accent:"#006bb6",accentLight:"#5ab4f0",
    badgeColor:"#006bb6",dotColor:"#5ab4f0",font:"Montserrat",palette:"azul Shimadzu",
    productQueries:["Shimadzu HPLC analytical instrument white background","Shimadzu spectrophotometer UV-Vis laboratory","Shimadzu analytical balance laboratory"],
  },
  dlab:{
    displayName:"DLAB",bg1:"#1a0000",bg2:"#5c0000",accent:"#cc0000",accentLight:"#ff4d4d",
    badgeColor:"#cc0000",dotColor:"#ff4d4d",font:"Montserrat",palette:"vermelho DLAB",
    productQueries:["DLAB pipette micropipette white background product","DLAB single channel adjustable pipette laboratory","DLAB multichannel pipette set laboratory"],
  },
  eppendorf:{
    displayName:"EPPENDORF",bg1:"#002e1f",bg2:"#005c3f",accent:"#00884a",accentLight:"#33cc7a",
    badgeColor:"#00884a",dotColor:"#33cc7a",font:"Montserrat",palette:"verde Eppendorf",
    productQueries:["Eppendorf Research Plus pipette white background","Eppendorf centrifuge laboratory equipment","Eppendorf ThermoMixer laboratory instrument"],
  },
  brand_generic:{
    displayName:"",bg1:"#030d1a",bg2:"#0a2d5e",accent:"#0057b8",accentLight:"#4da6ff",
    badgeColor:"#e8001c",dotColor:"#e8001c",font:"Montserrat",palette:"azul padrão",
    productQueries:null,
  },
};
...
async function runCopywriterAgent(prompt,intent,model,ctxRules=""){
  ...
  if(!res.ok){const txt=await res.text().catch(()=>"");throw new Error(`Agente 1 falhou (${res.status}): ${txt}`);}
...
function bannerTemplate(d){
  ...
  const FALLBACKS=["https://images.unsplash.com/photo-1532187863486-abf9dbad1b69?w=400&q=80","https://images.unsplash.com/photo-1581092160607-ee22621dd758?w=400&q=80","https://images.unsplash.com/photo-1554475901-4538ddfbccc2?w=400&q=80"];
...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    ollamaRes = await fetch(`${OLLAMA_URL}/api/generate`, {
      ...
      body:    JSON.stringify({ model, system: systemPrompt, prompt: userPrompt.trim(), stream: true }),
    });
  } catch (err) {
...
app.post("/api/translate", async (req, res) => {
  ...
  try {
    const r = await fetch(`${OLLAMA_URL}/api/generate`, {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ model, prompt:`Translate this marketing brief to concise vivid English for an image generator. Return ONLY the English prompt.\n\n${prompt}`, stream:false }),
    });
    if (!r.ok) return res.json({ englishPrompt: prompt });
    const data = await r.json();
    return res.json({ englishPrompt: (data.response??prompt).trim().replace(/^["']|["']$/g,"") });
...
app.post("/api/generate-banner", async (req,res) => {
  ...
  if(brand.productQueries){try{productImages=await searchProductImages(brand.productQueries);}catch(e){console.error("[banner]",e);}}
  ...
  let ollamaData;
  try{
    const or=await fetch(`${OLLAMA_URL}/api/generate`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model,prompt:schema,stream:false})});
    ollamaData=await or.json();
  }catch(e){return res.status(502).json({error:"Ollama indisponível"});}
  let fields={};
  try{const raw=(ollamaData.response||"").trim();const m=raw.match(/\{[\s\S]*\}/);if(m)fields=JSON.parse(m[0]);}catch(e){console.error("[banner] JSON:",e);}
  const d={...brand,brand:brand.displayName,bg_image_url:null,product_img_1:productImages[0],product_img_2:productImages[1],product_img_3:productImages[2],headline:fields.headline||"Precisão que transforma",highlight:fields.highlight||"",subline:fields.subline||"",description:fields.description||"",badge_value:fields.badge_value||"3",badge_sup:fields.badge_sup||"%",badge_label:fields.badge_label||"de Desconto",validity:fields.validity||"Oferta por tempo limitado",cta:fields.cta||"Ver Oferta"};
